//
// Adapted for Eyes app. Removed unsupported features (e.g., manual eye
//  movement, wink, etc.).
...
void drawEye( // Renders one eye.  Inputs must be pre-clipped & valid.
  ...
  uint16_t dlidX = -1;
  if (e) dlidX = 1;
  for (screenY = 0; screenY < EYE_HEIGHT; screenY++, scleraY++, irisY++) {
    scleraX = scleraXsave;
    irisX   = scleraXsave - (SCLERA_WIDTH - IRIS_WIDTH) / 2;
    if (e) lidX = 0; else lidX = EYE_WIDTH - 1;
    for (screenX = 0; screenX < EYE_WIDTH; screenX++, scleraX++, irisX++, lidX += dlidX) {
      if ((pgm_read_byte(lower + screenY * EYE_WIDTH + lidX) <= lT) ||
          (pgm_read_byte(upper + screenY * EYE_WIDTH + lidX) <= uT)) {              // Covered by eyelid
        p = 0;
...
void frame(uint16_t iScale) // Iris scale (0-1023)
{
  ...
  eyeX = map(eyeX, 0, 1023, 0, SCLERA_WIDTH  - EYE_WIDTH);
  eyeY = map(eyeY, 0, 1023, 0, SCLERA_HEIGHT - EYE_HEIGHT);
  ...
  if (eyeX > (SCLERA_WIDTH - EYE_WIDTH)) eyeX = (SCLERA_WIDTH - EYE_WIDTH);
  ...
  if (sampleY < 0) {
    ...
    n = (pgm_read_byte(upper + sampleY * EYE_WIDTH + sampleX) +
         pgm_read_byte(upper + sampleY * EYE_WIDTH + (EYE_WIDTH - 1 - sampleX))) / 2;
  }
